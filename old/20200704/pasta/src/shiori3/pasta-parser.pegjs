{
  ...
  const loc = ()=> ast.loc(location(), options);
}
...
// ２．１：コメント行
// [#]で始まり、改行の手前で終わる。
comment_line = PRI_COMMENT comment:comment_text &BR? { return ast.comment_line(comment, loc());}
comment_text = NOT_BR* {return text(); }
...
// ##### ####### ####### ####### ####### #######
// 以下、pegjs javascript grammerより抜粋
// 一般トークンに置き換えて利用すること
// ##### ####### ####### ####### ####### #######
